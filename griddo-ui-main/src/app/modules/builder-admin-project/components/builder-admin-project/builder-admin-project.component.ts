...
import { UnitDetailModalComponent } from '../unit-detail-modal/unit-detail-modal.component';

@Component({
  selector: 'app-builder-admin-project',
  templateUrl: './builder-admin-project.component.html',
  styleUrls: ['./builder-admin-project.component.scss']
})
export class BuilderAdminProjectComponent implements OnInit {
        ...
        tower?.projectBlocks?.forEach(block => {
          ...
          block?.projectFloorDetails?.forEach(floor => {
            ...
            floor?.projectUnitFlats?.forEach(flat => {
              ...
              flat['bgColor'] = indicator?.bgColor || (this.indicators.length ? this.indicators.find(i => i?.title?.toLowerCase() === "available")?.bgColor : '')
            })
          })
        })
      })
