...
@Component({
  ...
  styleUrls: ['./worksheet-modal.component.scss']
})
export class WorksheetModalComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<WorksheetModalComponent>, @Inject(MAT_DIALOG_DATA) public worksheet: any,
  ...
  init(res: any) {
    ...
    if (this.project.projectTower?.length) {
      console.log(this.project.projectTower);
      ...
      this.tower = { ...this.project.projectTower[0], isSeleted: false };
      this.isProgressing = false;
      //this.configStacks();
    }
  ...
  onSelectTower(tower: ProjectTower) {
    ...
    this.tower = { ...tower, isSelected: false };
    //this.configStacks();
  }


  selectUnit() {
    this.dialogRef.close({ flatNo: this.flatNo, flatId: this.flatId, flatPremiumPrice: this.flatPremiumPrice, flatPrice: this.flatPrice, tower: this.tower, flatDetail: this.flatDetail });
  }

  fetchUnit(flatId, flatNo) {
    this.flatNo = flatNo;
    this.dialogRef.close({ flatId: flatId, flatNo: flatNo, flatPremiumPrice: this.flatPremiumPrice, flatPrice: this.flatPrice });
  }

  openDialog(unitDetail: any) {
    this.flatNo = unitDetail.flatNo;
    this.flatId = unitDetail.flatId;
    this.flatPrice = unitDetail.price;
    this.flatPremiumPrice = unitDetail.floorPremium;
