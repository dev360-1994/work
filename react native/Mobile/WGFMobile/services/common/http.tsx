...
export const deleteAsync = (url: string, fileToken = "") =>
  new Promise((resolve, reject) => {
    getSession().then(function (value) {
      token = value && (value.token ? value.token : "");
    });
...
export const videoPost = async (url: string, data: FormData) => {
  let response;
  await axios({
    method: "post",
    url: url,
    ...
    // console.log("videoPost response" + res);
    response = res;
...
export const OnPutSubmitUserDetails = (url: string, data = {}, fileToken = "") =>
  new Promise((resolve, reject) => {
    getSession()
      .then(function (value) {
        token = value && (value.token ? value.token : "");
...
//Leave Team /api/User/LeaveTeam/{teamId}/{userId}

export const LeaveTeam = (url: string, fileToken = "") =>
...
// const profileUpdateSuccesfulToastr = () => {
...
export const postFilm = async (url: string, data: {}) => {
  let responseData;
  await getSession()
    .then(function (value) {
      data.TeamId = value.userInfo?.teamId.toString() ?? "0";
...
export const getTeamUserData = (url: string) => new Promise((resolve, reject) => {
  getSession()
    .then(function (value) {
      return value;
    })
    .then((value) => {
      token = value && (value.token ? value.token : "");
      url = url +
        `/${value.userInfo?.userId.toString() ?? "0"}/${value.userInfo?.teamId.toString() ?? "0"}`;
      let options = {
        headers: {
          Authorization: token ? `Bearer ${token}` : "",
          Accept: "application/json",
...
export const getTeamSessionStartData = (url: string) => new Promise((resolve, reject) => {
  getSession()
    .then(function (value) {
      return value;
    })
    .then((value) => {
      token = value && (value.token ? value.token : "");
      url = url +
        `/${value.userInfo?.teamId.toString() ?? "0"}`;
      let options = {
        headers: {
          Authorization: token ? `Bearer ${token}` : "",
          Accept: "application/json",
...
export const UpdateDashboardTeamColor = (url: string, data = {}, fileToken = "") =>
  new Promise((resolve, reject) => {
    getSession().then(function (value) {
      token = value && (value.token ? value.token : "");
    });
...
export const getHistoryData = (url: string) => new Promise((resolve, reject) => {
  getSession()
    .then(function (value) {
      return value;
    })
