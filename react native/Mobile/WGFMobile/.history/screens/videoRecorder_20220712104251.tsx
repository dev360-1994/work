...
export default function VideoRecorderScreen({ navigation }: RootDrawerScreenProps<'VideoRecord'>) {
  ...
  const takeVideo = async () => {
    ...
    if (camera) {
      ...
      await uploadVideo(fsRead, videoId, extension).then(() => { setUploadingCount(uploadingCount + 1) });
        ...
        <View style={styles.centeredView}>
          ...
          >
            <View style={{ flex: 1, }}>
              ...
              >
                <Text style={{ color: "white", fontSize: 15 }} >Uploading {uploadingCount} of {recordingCount}</Text>
              </View>
