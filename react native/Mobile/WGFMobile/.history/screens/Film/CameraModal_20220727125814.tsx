...
export const VideoRecorderModal = (props) => {
    ...
    const takeVideo = async () => {
        ...
        console.log(clipOrder);
        setShowStart(false);
        setRecordingCount(recordingCount + 1);
        setUploadedCount(prevState => prevState == 0 ? 1 : prevState + 1);
        if (camera) {
            ...
            data.append("clipOrder", clipOrder);
        ...
        <View style={{ flex: 1, width: "100%", height: "100%" }}>
            ...
            <View >
                <View style={[styles.titleText, { backgroundColor: userTeamColor, opacity: getScreen() ? 0.4 : 1 }]}>
                    <Text style={{ color: "white", fontSize: 15, textAlign: 'center', paddingTop: 2 }} >{props?.route?.params?.filmTitle}{props?.route?.params?.title}</Text>
                </View>
            </View>
