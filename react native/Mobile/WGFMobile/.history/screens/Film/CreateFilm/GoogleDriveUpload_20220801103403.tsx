...
export default function GoogleDriveUpload({ navigation }) {
    ...
    const GoogleDriveFiles = (folderId: string) => {
        var result = new List<GoogleDriveFiles>();
        try {
            ...
            do {
                ...
                foreach(var file in (JArray)contents["files"])
                {
                    ...
                    if (!string.IsNullOrEmpty(extn)) {
                        result.Add(new GoogleDriveFiles
                            {
                                Id = (string)file["id"],
        ...
        return new JsonResult(result.OrderBy(x => x.Name).ToList());
    }
