...
export default function DashboardScreen({ navigation }: RootDrawerScreenProps<'Dashboard'>) {
    ...
    const [themeMode, setThemeMode] = useState(userContext.isThemeDark);
            ...
            headerRight: () => (
                <Pressable
                    ...
                    <OptionsMenu
                        ...
                        />}
                        // destructiveIndex={0}
                        options={[themeMode ? "Dark Mode" : "Light Mode", "Cancel"]}
                        actions={[() => { toggleTheme(); setThemeMode(userContext.isThemeDark ? true : false) }]} />
                </Pressable>
            ),
        })
    }, [navigation]);
