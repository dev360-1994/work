...
export default function TeamDetailsScreen({ navigation }: RootDrawerScreenProps<'TeamDetails'>) {
    ...
    const [teamUserInfo, setTeasmUserInfo] = useState([]);
    ...
    useLayoutEffect(() => {
        async function fetchTeamDetails() {
            new Promise((resolve, reject) => {
                getSession().then(async (value) => {
                    setTeamId(value?.userInfo?.teamId);
                    setUserId(value?.userInfo?.userId);

                }).then(async () => {
                    const response = await getSettingsTeamUser(userId, teamId);
                    if (response) {
                        setTeasmUserInfo(response);
                    }
        ...
        setTeamName(teamUserInfo.teamName);
    });
