...
export default function TeamDetailsScreen({ navigation }: RootDrawerScreenProps<'TeamDetails'>) {
    ...
    useEffect(() => {
        const fetchSeason = async () => {
            const response = await getTeamSessionStart().then((response) => {
                setSeason(response); console.log(response);
                setValue(response);
            });
        }
        fetchSeason();
        // console.log(season);
    }, [getTeamSessionStart, setValue])
            ...
            headerRight: () => (
                <Pressable onPress={openMenu}
                    ...
                    <Menu
                        ...
                        />
                        <Menu.Item onPress={() => { closeMenu(); setValue(season); showColorDialog(); }} title={
                            <Button
                                color={userContext.isThemeDark ? "#fff" : "#000"}
                            >Team Color</Button>
                        }
