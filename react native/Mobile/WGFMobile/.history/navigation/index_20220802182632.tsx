...
function DrawerNavigator() {
  ...
  const userRole = getUserRole(userContext.userInfo.userRole).toLowerCase();
  const run = async () => {
    const result = await getSecurityData(userContext.userInfo.teamId);
    result.forEach(element => {
      if (element.categoryDesc === "Film" && element.actionDesc == "Upload") {
        setIsDrawerVisible(element[userRole] == 1);
        console.log("inside: ", isDrawerVisible)
      }
    });
  ...
  console.log("outside", isDrawerVisible);
