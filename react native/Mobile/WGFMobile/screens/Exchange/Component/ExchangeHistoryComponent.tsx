...
import { exchangeHistoryGridData } from "../../../services/ExchangeService";
import { ExchangeHistory } from "../../../services/Model/ExchangeModel";
import { MessageModel } from "../../../services/Model/MessageModel";
import { SessionInfoModel } from "../../../services/Model/SessionInfoModel";

export const ExchangeHistoryComponent = ({ sessionData }: { sessionData?: SessionInfoModel }) => {
    const [searchPlaceHolder, setSearchPlaceHolder] = useState("History");
    const theme = useTheme();
    const [filterdataSource, setFilterDataSource] = React.useState<ExchangeHistory[]>([]);
    const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
    const [message, setNotification] = useState<MessageModel>(new MessageModel());
    const [dataSource, setDataSource] = React.useState<ExchangeHistory[]>([]);
    ...
    async function fetchPlaylists() {
        ...
        try {
            ...
            var resultPromise = await exchangeHistoryGridData(params);
            if (resultPromise) {
                let result: ExchangeHistory[] = JSON.parse(JSON.stringify(resultPromise));
    ...
    const onPressViewFilmHandler = async (item: ExchangeHistory) => {
    };
                    ...
                    renderItem={({ item }) => (
                        <View style={[styles.wrapper, theme.dark ? styles.borderForDarkTheme : styles.borderForLightTheme, { backgroundColor: theme.colors.surface }]}>
                            ...
                            >
                                ...
                                {
                                    item.sendTeam != null &&
                                    <Text style={{ color: Colors.grey600 }}>{item.sendTeam}</Text>
                                }
                                {
                                    item.receiveTeam != "" &&
                                    <Text style={{ color: Colors.grey600 }}>{item.receiveTeam}</Text>
                                }
                                {
                                    item.sender != "" &&
                                    <Text style={{ color: Colors.grey600 }}>{item.sender}</Text>
                                }
                                {
                                    item.filmDate &&
