...
export const OpenComponent = ({ sessionData }: { sessionData?: SessionInfoModel }) => {
    const [searchPlaceHolder, setSearchPlaceHolder] = useState("Open Exchange");
    ...
    const onPressViewFilmHandler = async (item: ExchangeModel) => {
        ...
        navigation.navigate('OpenForm', {
            title: item.playlistname, date: item.playlistdate, filmGuid: item.playlistguid, playlistId: result.playlistid, clips: result.clips, status: result.status
        });
    };
