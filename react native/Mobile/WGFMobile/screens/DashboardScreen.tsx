...
import { Chip, Switch, useTheme, Colors, Menu, Button, Divider } from 'react-native-paper';
...
export default function DashboardScreen({ navigation }: RootDrawerScreenProps<'Dashboard'>) {
    ...
    const [roleName, setRoleName] = useState<string>("Owner");
            ...
            headerRight: () => (
                <Pressable onPress={openMenu}
                    ...
                    <Menu
                        ...
                        <Menu.Item onPress={toggleMenu} title={(userContext.isThemeDark ? "Light" : "Dark") + " Mode"} />
                    </Menu>
                </Pressable>
            ),
        })
    ...
    useLayoutEffect(() => {
        const getUserSecurityData = async () => {
            ...
            if (resultPromise) {
                ...
                if (sessionData?.userInfo?.teamId) {
                    setRoleName(result[sessionData?.userInfo.userRole].roleValue)
                }
            }
        }
        getUserSecurityData();
    ...
    async function fetchUserRoles(userSessionData: SessionInfoModel) {
        try {
            ...
            //         if (sessionData?.userInfo.userRole) {
            //             setRoleName(result[sessionData?.userInfo.userRole].roleValue);
            //         }
    ...
    async function fetchUserTeamInfo(userSessionData: SessionInfoModel) {
        try {
            ...
            // console.log("responseUserTeamInfo: ", responseUserTeamInfo);
