...
const PlaylistComponent = ({ typeCode, typeName, sessionData }: { typeCode: string, typeName: string, sessionData?: SessionInfoModel }) => {
    ...
    useEffect(() => {
        const getUserSecurityData = async () => {
            const result: TeamUserSecurity = await teamUserLayoutAction(sessionData?.userInfo.teamId, sessionData?.userInfo.userId, "film_upload");
            setShowVideoMenu(result.value);
        }
        getUserSecurityData();
    }, [sessionData])
        ...
        if (searchText != "") {
            const newData = dataSource.filter(
                function (item) {
                    ...
                    return itemData.indexOf(textData) > -1 || itemData2.indexOf(textData) > -1 || itemData1.indexOf(textData) > -1;
                });
    ...
    const onPressViewFilmHandler = async (item: PlaylistModel) => {
        navigation.navigate('ViewFilm', {
            screen: 'VIDEO',
            params: {
                playlist: item.playlist, view: item.view, clipId: 0, order: 1,
                title: item.name, showMenu: showVideoMenu, playlistData: item
            },
        });
    };
                        ...
                        <View style={[styles.wrapper, theme.dark ? styles.borderForDarkTheme : styles.borderForLightTheme, { backgroundColor: theme.colors.surface }]}>
                            ...
                            >
                                ...
                                }
                                {
                                    ...
                                    <Text style={{ color: Colors.grey600 }}>{new Date(item.dateExpiry)}</Text>
                                }
                                {
                                    item.group != "" &&
