...
export default function FavoritePlaylistScreen({ navigation }: RootDrawerScreenProps<'ViewPlaylist'>) {
    const isMountedRef = useIsMountedRef();
    const [sessionData, setLocalSessionData] = useState<SessionInfoModel | null>(null);
    ...
    const init = () => {
        try {
            ...
            getSession().then((s) => {
                ...
                setLocalSessionData(sessionInfo);
    ...
    return (
        <SafeAreaView style={styles.container}>
            {(isSubmitting) ? (
                ...
                <PlaylistComponent typeCode={'F'} typeName={'Favorite'} sessionData={sessionData ?? undefined} />
            )}
        </SafeAreaView>
    );
}
