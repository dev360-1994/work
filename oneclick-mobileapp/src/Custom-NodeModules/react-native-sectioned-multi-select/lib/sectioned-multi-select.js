...
const defaultColors = {
  ...
  searchPlaceholderTextColor: '#424242',
...
class SectionedMultiSelect extends PureComponent {
  static propTypes = {
    ...
    showAddOption: PropTypes.bool.isRequired,
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      addOption: false, exactMatch: false, newOption: '', renderData: []
    }
  }
  ...
  _getSelectLabel = () => {
    const {
      ...
      renderSelectText,
      selectLabelNumberOfLines, showAddOption
    } = this.props
    ...
    if (showAddOption && this.props.newOption) {
      label = ''
    }
  ...
  _filterItems = (searchTerm) => {
    ...
    const {
      items, subKey, uniqueKey, displayKey, filterItems, showAddOption
    } = this.props
    ...
    items &&
      items.forEach((item) => {
        const parts = !showAddOption ? searchTerm.replace(/[\^$\\.*+?()[\]{}|]/g, '\\$&').trim().split(' ')
          : searchTerm.replace(/[\^$\\.*+?()[\]{}|]/g, '\\$&').trim().split(',')
        ...
        if (item[subKey]) {
          ...
          item[subKey].forEach((sub) => {
            if (regex.test(this.getProp(sub, displayKey))) {
              ...
              var t = sub["name"];
              if (showAddOption && searchTerm.trim().toLowerCase() === sub["name"].toLowerCase()) {
                this.setState({ addOption: false, exactMatch: true })
              }
              else if (showAddOption && searchTerm.trim().toLowerCase() !== sub["name"].toLowerCase()) {
    ...
    if (filteredItems.length == 0 && showAddOption) {
      this.setState({ addOption: true })
  ...
  _toggleItem = (item, hasChildren) => {
    ...
    if (single) {
      ...
      if (item["name"] === undefined && this.state.newOption == item) {
  ...
  _displaySingleItems = () => {
    ...
    const item = this.state.newOption;
    const isParent = true;
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          onPress={() => {
            isParent && parentChipsRemoveChildren
              ? this._toggleChildren(item, undefined)
              : this._toggleItem(item, undefined)
          }}
          style={{
            borderTopRightRadius: 20,
            borderBottomRightRadius: 20,
  ...
  _renderNewOption = (searchText) => {
    const { onAddOption } = this.props;
    this.setState({ newOption: searchText, searchTerm: "", renderData: [], addOption: false })
    this._toggleSelector()
    setTimeout(() => {
      onAddOption(true, searchText)
    }, 30);
  ...
  searchItems = (text) => {
    this.setState({ searchTerm: text, renderData: this._filterItems(text.trim()) });
  }
  ...
  render() {
    const {
      ...
      disabled,
      itemsFlatListProps,
      showAddOption, newOption
    } = this.props
    const { searchTerm, selector, styles, colors, renderData } = this.state
    const renderItems = renderData.length > 0 ? renderData : searchTerm.length == 0 ? items : []; //searchTerm ? this._filterItems(searchTerm.trim()) : items
      ...
      <View>
            ...
            >
                ...
                <View style={[{ flexDirection: 'row', paddingVertical: 5 }, styles.searchBar]}>
                  <View style={styles.center}>
                    {callIfFunction(searchIconComponent) || (
                      <Icon name="search" size={20} style={{ marginHorizontal: 15 }} />
                    )}
                  </View>
                  <TextInput
                    value={this.state.searchTerm}
                    selectionColor={colors.searchSelectionColor}
                    onChangeText={searchTerm => this.searchItems(searchTerm)}//{searchTerm => this.setState({ searchTerm })}
              ...
              >
                <View>
                  ...
                  {showAddOption && this.state.addOption && !this.state.exactMatch && <View style={{ marginTop: 10, flexDirection: 'row' }}>
                    <TouchableOpacity onPress={() => this._renderNewOption(searchTerm)}
                      style={{
                        flexDirection: 'row',
                        alignItems: 'center',
                        color: '#424242'
                    ...
                    >
                      ...
                      <Text style={{ color: '#424242', fontSize: 14 }}>
                        Add New Item : "{this.state.searchTerm}"
                              </Text>
                    </TouchableOpacity>
                  </View>
                  }
              ...
              <View style={{ flexDirection: 'row' }}>
                ...
                {!hideConfirm && (
                  ...
                  >
                    ...
                    >
                      <Text
                        style={[{ fontSize: 15, color: '#ffffff' }, confirmFont, styles.confirmText]}
                      >
                        {confirmText}
                      </Text>
                    </View>
        ...
        {!hideSelect && (
          ...
          >
            ...
            >
              {this._displaySelectedItems()}
              {(newOption !== '' && newOption !== undefined) && this._displaySingleItems()}
              {/* {this.state.newOption !== '' && this._displaySingleItems()} */}
              {selectedItems.length === 0 && this._getSelectLabel()}
            </View>
          </TouchableWithoutFeedback>
        )}
