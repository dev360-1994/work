...
export default class TreeSelect extends React.Component {
  ...
  async UNSAFE_componentWillMount() {
    ...
    this.setState({ selectedKeysMap, expandedMap: { ...this.state.expandedMap, expandedMap } });
  }
  async UNSAFE_componentWillReceiveProps (nextProps, nextState){
    const { value, multiple, treeDefaultExpandedKeys, treeDefaultExpandAll } = this.props;
    const { predecessorsOfNodeMap, ownedLeafNodesMap, allNodesMap } = Utils.getPredecessorsMap(nextProps.treeData);
    let leafNodesSelectedKeysMap = {};
    let predecessorsSelectedKeysMap = {};
    this.setState({a:true})
    if (multiple) {
      leafNodesSelectedKeysMap = Utils.arrayToObj(nextProps.value, ownedLeafNodesMap);
      Utils.traverseTree(nextProps.treeData, (node) => {
        const { children, key } = node;
        if (children) {
          const { count, keysMap } = ownedLeafNodesMap[key];
          const selectedCount = Object.keys(leafNodesSelectedKeysMap).filter(k => keysMap[k]).length;
  ...
  async addChild(nodeData,key){
  this.setState({ expandedMap: { ...this.state.expandedMap, [key]: true } } ,function(){
    this.props.addChild(nodeData,key,this.state.valuenode)
  });
  }
  getSelectValue = () => {
  ...
  renderNode(nodeData) {
    const { onlyCheckLeaf, multiple ,addChild,valuedata } = this.props;
   ...
   const predecessorsCount = this.predecessorsOfNodeMap[key].length;
    return (
      <TreeNode
        ...
        nodeData={nodeData}
        addChild={(nodeData)=>this.addChild(nodeData,key,this.state.valuenode)} 
        valuedata={()=>valuedata()}
        />
    );
  }
  ...
  renderTree(children, parentExpanded) {
    ...
    return children.map((nodeData) => {
      ...
      return (
        <View key={key} style={{
          //height: parentExpanded ? 'auto' : 'auto',
